...
public class QuadraticRegression {
    ...
    public void calculateParameters() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            X[i][2] = dataset.getX().get(i) * dataset.getX().get(i); // Columna de x^2 para beta_2
    ...
    public double calculateRSquared(List<Double> testX, List<Double> testY) {
        ...
        for (int i = 0; i < testX.size(); i++) {
            ...
            ssTotal += (testY.get(i) - meanY) * (testY.get(i) - meanY); // Diferencia entre el valor real y la media de Y
            ssResidual += (testY.get(i) - predictedY) * (testY.get(i) - predictedY); // Diferencia entre el valor real y el predicho
        }
